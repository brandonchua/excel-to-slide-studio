...
function buildSpec(cfg){
  ...
  const spec={meta:{source:STATE.wbName,cutoff:cfg.cutoff,currency:"S$M",generated:new Date().toISOString().slice(0,16).replace("T"," ")},
    theme:{primary:tpl.primary,accent:tpl.accent,dark:tpl.dark,brandName:"SP GROUP",template:"sp",font:"Arial",logo:null,logoAspect:null},
...
function logoTag(sp){const src=(sp&&sp.theme&&sp.theme.logo)||LOGO;return `<img src="${src}" style="position:absolute;top:40px;right:60px;height:60px;width:auto" alt="logo">`;}
function headerEl(el,sp,title,sub){const theme=sp.theme;el.insertAdjacentHTML("beforeend",logoTag(sp));
...
function renderSlide(el,sl,sp){
  ...
  el.style.fontFamily="'"+((theme.font)||"Arial")+"', Arial, 'Segoe UI', sans-serif";
  if(sl.type==="title"){el.style.background="#fff";
    el.innerHTML=`${logoTag(sp)}
...
function applyOps(ops){const sp=STATE.spec;ops.forEach(o=>{try{
  ...
  else if(o.op==="set_template"){const tpl=TEMPLATES[o.name];if(tpl){sp.theme.primary=tpl.primary;sp.theme.accent=tpl.accent;sp.theme.dark=tpl.dark;sp.theme.template=o.name;sp.theme.font="Arial";sp.theme.logo=null;sp.theme.logoAspect=null;syncBrandBar();}}
  else if(o.op==="apply_imported"){const t=o.tpl;sp.theme.primary=t.primary;sp.theme.accent=t.accent;sp.theme.dark=t.dark;sp.theme.font=t.font||"Arial";sp.theme.logo=t.logo||null;sp.theme.logoAspect=t.logoAspect||null;sp.theme.template=t.id;syncBrandBar();}
...
/* ---------- custom PowerPoint templates (in-browser import) ---------- */
function loadTpls(){try{return JSON.parse(localStorage.getItem("x2s_tpls")||"{}");}catch(e){return{};}}
function saveTpls(t){try{localStorage.setItem("x2s_tpls",JSON.stringify(t));}catch(e){}}
function refreshTplOptions(){const sel=$("tplSel");if(!sel)return;const base='<option value="sp">SP Classic (teal / navy)</option><option value="light">Light Minimal</option><option value="exec">Midnight Executive</option><option value="energy">Energy (green)</option>';const custom=Object.values(loadTpls()).map(t=>`<option value="${t.id}">${esc(t.name)} (imported)</option>`).join("");sel.innerHTML=base+custom;}
function themeColor(xml,tag){const m=xml.match(new RegExp("<a:"+tag+">([\\s\\S]*?)</a:"+tag+">"));if(!m)return null;const s=m[1].match(/<a:srgbClr val="([0-9A-Fa-f]{6})"/);if(s)return s[1].toUpperCase();const sys=m[1].match(/lastClr="([0-9A-Fa-f]{6})"/);return sys?sys[1].toUpperCase():null;}
async function parsePptxTemplate(buf){
  if(typeof JSZip==="undefined")throw new Error("template engine still loading, try again in a moment");
  const zip=await JSZip.loadAsync(buf);const names=Object.keys(zip.files);
  let primary="0097A7",accent="FFAB40",dark="282F45",font="Arial";
  const tf=names.find(n=>/^ppt\/theme\/theme1\.xml$/.test(n))||names.find(n=>/^ppt\/theme\/theme\d+\.xml$/.test(n));
  if(tf){const xml=await zip.file(tf).async("string");
    primary=themeColor(xml,"accent1")||primary;accent=themeColor(xml,"accent2")||accent;dark=themeColor(xml,"dk2")||themeColor(xml,"dk1")||dark;
    const fm=xml.match(/<a:majorFont>[\s\S]*?<a:latin typeface="([^"]+)"/);if(fm&&fm[1]&&fm[1].indexOf("+")!==0)font=fm[1];}
  const media=names.filter(n=>/^ppt\/media\/.*\.(png|jpe?g)$/i.test(n)).sort();
  let logo=null,logoAspect=null;
  if(media.length){const ext=media[0].split(".").pop().toLowerCase();const b64=await zip.file(media[0]).async("base64");logo="data:image/"+(ext==="jpg"?"jpeg":ext)+";base64,"+b64;
    logoAspect=await new Promise(res=>{const im=new Image();im.onload=()=>res(im.width/im.height);im.onerror=()=>res(null);im.src=logo;});}
  return {primary,accent,dark,font,logo,logoAspect};
}

/* ---------- theme + settings popovers ---------- */
function syncBrandBar(){const th=STATE.spec.theme;$("brandName").value=th.brandName;$("cPrimary").value="#"+th.primary;$("cAccent").value="#"+th.accent;$("cDark").value="#"+th.dark;refreshTplOptions();$("tplSel").value=th.template||"sp";}
function wireBrandBar(){
  ...
  $("tplSel").addEventListener("change",e=>{const v=e.target.value;const tpls=loadTpls();if(tpls[v])aiApply([{op:"apply_imported",tpl:tpls[v]}]);else aiApply([{op:"set_template",name:v}]);});
  const tf=$("tplFile");if(tf)tf.addEventListener("change",async()=>{const f=tf.files[0];if(!f)return;overlay(true,"Reading template…");
    try{const buf=await f.arrayBuffer();const t=await parsePptxTemplate(buf);const id="tpl"+Date.now();const name=(f.name.replace(/\.pptx$/i,"")||"Imported").slice(0,40);const tpls=loadTpls();tpls[id]=Object.assign({id,name},t);saveTpls(tpls);overlay(false);
      aiApply([{op:"apply_imported",tpl:tpls[id]}]);refreshTplOptions();$("tplSel").value=id;
      $("tplMsg").textContent=`Applied “${name}” — colours updated, font ${t.font}${t.logo?", logo imported":""}. Click Save to keep it.`;}
    catch(err){overlay(false);alert("Could not read this template: "+(err.message||err));}
    tf.value="";});}
...
function exportPptx(){
 try{
  ...
  const FF=th.font||"Arial";const lAsp=th.logoAspect||(275/108);const lh=0.62,lw=lh*lAsp;
  const foot=`Auto-generated from ${sp.meta.source} · cut-off ${sp.meta.cutoff} · figures in ${sp.meta.currency} · DRAFT`;
  const logoImg=s=>{try{s.addImage({data:th.logo||LOGO,x:12.73-lw,y:0.33,w:lw,h:lh});}catch(e){}};
  const hdr=(s,title,sub)=>{logoImg(s);
    s.addText(title||"",{x:0.7,y:0.5,w:9.8,h:0.7,fontSize:24,bold:true,color:th.dark,fontFace:FF,valign:"middle"});
    s.addShape("rect",{x:0.7,y:1.22,w:11.93,h:0.04,fill:{color:th.primary}});
    if(sub)s.addText(sub,{x:0.7,y:1.32,w:12,h:0.35,fontSize:12,color:"5C5C5C",fontFace:FF});};
  ...
  sp.slides.forEach((sl,idx)=>{const s=p.addSlide();
    if(sl.type==="title"){logoImg(s);
      s.addText(sl.title,{x:0.9,y:2.9,w:11,h:1.1,fontSize:40,bold:true,color:th.dark,fontFace:FF});
      s.addShape("rect",{x:0.92,y:4.05,w:4.6,h:0.06,fill:{color:th.primary}});
      s.addText(sl.subtitle||"",{x:0.9,y:4.2,w:11,h:0.5,fontSize:20,color:th.primary,fontFace:FF});
      s.addText(`Cut-off ${sp.meta.cutoff} · generated ${sp.meta.generated}`,{x:0.9,y:6.6,w:11,h:0.4,fontSize:12,color:"8A939B",fontFace:FF});return;}
    ...
    if(sl.type==="kpi"){let x=0.7,w=2.95;(sl.cards||[]).slice(0,4).forEach(c=>{s.addShape("rect",{x,y:2.4,w,h:2.0,fill:{color:"E8F6F8"},line:{color:th.primary,width:1.5}});
      s.addText(c.label,{x:x+0.2,y:2.55,w:w-0.4,h:0.4,fontSize:13,bold:true,color:th.primary,fontFace:FF});
      s.addText(String(c.value),{x:x+0.2,y:2.95,w:w-0.4,h:0.8,fontSize:34,bold:true,color:th.dark,fontFace:FF});
      s.addText(c.note||"",{x:x+0.2,y:3.85,w:w-0.4,h:0.4,fontSize:10,color:"5C5C5C",fontFace:FF});x+=w+0.18;});}
      ...
      s.addTable([head,...body],{x:0.7,y:1.95,w:11.95,fontSize:10,fontFace:FF,border:{type:"solid",color:"FFFFFF",pt:1},colW:[3.0,...sl.fy.map(()=>(11.95-3.0)/sl.fy.length)],rowH:0.34,valign:"middle"});}
    else if(sl.type==="changes"){const list=(sl.changes||[]).slice(0,11);
      if(!list.length){s.addText("No material changes versus the prior version.",{x:0.9,y:3,w:11,h:1,fontSize:18,color:"5C5C5C",fontFace:FF});}
        ...
        s.addTable([head,...body],{x:0.7,y:1.95,w:11.95,fontSize:11,fontFace:FF,border:{type:"solid",color:"FFFFFF",pt:1},colW:[5.5,2.14,2.14,2.15],rowH:0.4,valign:"middle"});}}
    else if(sl.type==="text"){s.addText((sl.bullets||[]).map(b=>({text:b,options:{bullet:{code:"2022"},color:th.dark,fontSize:18,paraSpaceAfter:10}})),{x:0.9,y:2.3,w:11.5,h:4,fontFace:FF});}
    footer(s);});
...
function renderWorkspace(){
  ...
  side.innerHTML=series.map(s=>{const vs=seriesVersions(L,s.id);const lv=latestVersion(L,s.id);
    ...
    return `<div class="srcitem ${s.id===STATE.selSeries?"sel":""}">
      <button class="srcsel" data-act="selsrc" data-id="${s.id}"><span class="srcname">${esc(s.name)}</span><span class="srcmeta">${vs.length}v</span>${upd?'<span class="dot-upd" title="updates available"></span>':''}</button>
      <button class="srcic" data-act="rensrc" data-id="${s.id}" title="Rename source">✎</button>
      <button class="srcic del" data-act="delsrc" data-id="${s.id}" title="Delete source">🗑</button>
    </div>`;}).join("");
  ...
  let h=`<div class="ws-head"><div><h2>${esc(s.name)}</h2><div class="muted">Latest ${esc(latest?latest.label:"")} · ${vs.length} version(s) · ${decks.length} deck(s)</div>
      <div class="srcmanage"><button class="linkbtn rn" data-act="rensrc" data-id="${s.id}">Rename source</button><button class="linkbtn" data-act="delsrc" data-id="${s.id}">Delete source</button></div></div>
  ...
  h+=`<div class="seclabel">Version history</div><div class="timeline">`+vs.map((v,i)=>`<div class="vnode ${latest&&v.id===latest.id?"latest":""}"><div class="vdot"></div><div class="vlabel">${esc(v.label)}</div><div class="vdate">${new Date(v.capturedAt).toLocaleDateString()}</div><div class="vcount">${decks.filter(d=>d.boundVersionId===v.id).length} deck(s)</div><div class="vacts"><button class="vmini" data-act="renver" data-id="${v.id}" title="Rename version">✎</button><button class="vmini del" data-act="delver" data-id="${v.id}" title="Delete version">🗑</button></div></div>${i<vs.length-1?'<div class="vconn"></div>':''}`).join("")+`</div>`;
  ...
  h+=`<div class="seclabel" style="margin-top:26px">Decks</div><div class="deckgrid">`;
  if(!decks.length)h+=`<div class="muted">No decks yet for this source. Click “New deck” to create one.</div>`;
  decks.forEach(d=>{const st=deckStatus(L,d);const bv=L.versions[d.boundVersionId];
    const stTxt=st.k==="pin"?"Pinned":(st.k==="upd"?"Update available":"Up to date");
    const verLine=st.k==="pin"?`Pinned to ${esc(bv?bv.label:"?")}`:(st.k==="upd"?`Built from ${esc(bv?bv.label:"?")} · ${esc(latest.label)} available`:`Up to date · ${esc(latest?latest.label:"")}`);
    const primary=st.k==="upd"?`<button class="btn primary block" data-act="review" data-id="${d.id}">Review &amp; update</button>`:`<button class="btn primary block" data-act="open" data-id="${d.id}">Open</button>`;
    h+=`<div class="deckcard">
      <div class="dc-head"><div class="dc-name">${esc(d.name)}</div><div class="dc-right"><span class="stp ${st.k}">${stTxt}</span><button class="kebab" data-act="menu" data-id="${d.id}" title="More">⋯</button></div></div>
      <div class="dc-version ${st.k}"><span class="vico"></span>${verLine}</div>
      ${primary}
      <div class="cardmenu hidden" data-menu="${d.id}">
        ${st.k==="upd"?`<button class="mi" data-act="open" data-id="${d.id}">Open without updating</button>`:""}
        <button class="mi" data-act="rename" data-id="${d.id}">Rename…</button>
        <div class="mlabel">When a new version arrives</div>
        <button class="mi ${d.policy==="notify"?"on":""}" data-act="pol" data-pol="notify" data-id="${d.id}">Notify me</button>
        <button class="mi ${d.policy==="auto"?"on":""}" data-act="pol" data-pol="auto" data-id="${d.id}">Update automatically</button>
        <button class="mi ${d.policy==="pinned"?"on":""}" data-act="pol" data-pol="pinned" data-id="${d.id}">Keep pinned (never change)</button>
        <div class="mdiv"></div>
        <button class="mi danger" data-act="del" data-id="${d.id}">Delete deck</button>
      </div></div>`;});
  h+=`</div>`;
  $("wsMain").innerHTML=h;}
...
function closeMenus(){document.querySelectorAll(".cardmenu").forEach(m=>m.classList.add("hidden"));document.querySelectorAll(".deckcard").forEach(c=>{c.style.zIndex="";});}
$("workspace").addEventListener("click",e=>{const t=e.target.closest("[data-act]");if(!t)return;const act=t.getAttribute("data-act"),id=t.getAttribute("data-id");
  if(act==="menu"){e.stopPropagation();const m=document.querySelector(`.cardmenu[data-menu="${id}"]`);const wasHidden=m&&m.classList.contains("hidden");closeMenus();if(m&&wasHidden){m.classList.remove("hidden");const c=m.closest(".deckcard");if(c)c.style.zIndex="30";}return;}
  ...
  else if(act==="pol"){const L=loadLedger();const d=L.decks[id];if(!d)return;d.policy=t.getAttribute("data-pol");saveLedger(L);if(d.policy==="auto"){const lv=latestVersion(L,d.seriesId);if(lv&&d.boundVersionId!==lv.id)applyVersionToDeck(id,true);}renderWorkspace();}
  else if(act==="rename"){const d=loadLedger().decks[id];uiPrompt("Rename deck",d.name,v=>{const L=loadLedger();L.decks[id].name=v;saveLedger(L);renderWorkspace();});}
  else if(act==="del"){const d=loadLedger().decks[id];uiConfirm("Delete deck",`Delete “${d.name}”? This cannot be undone.`,()=>{const L=loadLedger();delete L.decks[id];saveLedger(L);renderWorkspace();},"Delete");}
  else if(act==="rensrc"){const s=loadLedger().series[id];uiPrompt("Rename source",s.name,v=>{const L=loadLedger();L.series[id].name=v;saveLedger(L);renderWorkspace();});}
  else if(act==="delsrc"){const L0=loadLedger();const s=L0.series[id];const nd=Object.values(L0.decks).filter(d=>d.seriesId===id).length;uiConfirm("Delete source",`Delete “${s.name}”, its ${seriesVersions(L0,id).length} version(s) and ${nd} deck(s)? This cannot be undone.`,()=>{const L=loadLedger();Object.values(L.versions).filter(v=>v.seriesId===id).forEach(v=>delete L.versions[v.id]);Object.values(L.decks).filter(d=>d.seriesId===id).forEach(d=>delete L.decks[d.id]);delete L.series[id];if(STATE.selSeries===id)STATE.selSeries=null;saveLedger(L);renderWorkspace();},"Delete");}
  else if(act==="renver"){const v=loadLedger().versions[id];uiPrompt("Rename version",v.label,nv=>{const L=loadLedger();L.versions[id].label=nv;saveLedger(L);renderWorkspace();});}
  else if(act==="delver"){const L0=loadLedger();const v=L0.versions[id];const bound=Object.values(L0.decks).filter(d=>d.boundVersionId===id);if(bound.length){uiConfirm("Can't delete version",`${bound.length} deck(s) are still on ${v.label}. Update or delete those decks first.`,null,"OK");}else{uiConfirm("Delete version",`Delete ${v.label}? This cannot be undone.`,()=>{const L=loadLedger();delete L.versions[id];saveLedger(L);renderWorkspace();},"Delete");}}});
document.addEventListener("click",e=>{if(!e.target.closest(".cardmenu")&&!e.target.closest(".kebab"))closeMenus();});

/* styled prompt / confirm dialogs (replace native popups) */
let _dlgOk=null;
function closeDlg(){show($("dlg"),false);_dlgOk=null;}
function uiConfirm(title,msg,onYes,okLabel){$("dlgTitle").textContent=title;$("dlgMsg").textContent=msg;$("dlgMsg").classList.remove("hidden");$("dlgInput").classList.add("hidden");$("dlgOk").textContent=okLabel||"Confirm";$("dlgOk").className="btn primary"+(okLabel==="Delete"?" danger-btn":"");show($("dlgCancel"),onYes!=null);_dlgOk=()=>{closeDlg();onYes&&onYes();};show($("dlg"),true);}
function uiPrompt(title,val,onOk){$("dlgTitle").textContent=title;$("dlgMsg").classList.add("hidden");const inp=$("dlgInput");inp.classList.remove("hidden");inp.value=val||"";$("dlgOk").textContent="Save";$("dlgOk").className="btn primary";show($("dlgCancel"),true);_dlgOk=()=>{const v=inp.value.trim();closeDlg();if(v)onOk&&onOk(v);};show($("dlg"),true);setTimeout(()=>{inp.focus();inp.select();},30);}
$("dlgX").addEventListener("click",closeDlg);
$("dlgCancel").addEventListener("click",closeDlg);
$("dlg").addEventListener("click",e=>{if(e.target===$("dlg"))closeDlg();});
$("dlgOk").addEventListener("click",()=>{if(_dlgOk)_dlgOk();});
$("dlgInput").addEventListener("keydown",e=>{if(e.key==="Enter"&&_dlgOk)_dlgOk();});
